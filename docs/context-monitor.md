...
| CRITICAL | <= 25% | Stop immediately, save state (`/maxsim:pause-work`) |
...
```
Statusline Hook (maxsim-statusline.js)
    | writes
    v
/tmp/claude-ctx-{session_id}.json
    ^ reads
    |
Context Monitor (maxsim-context-monitor.js, PostToolUse)
    | injects
    v
...
MAXSIM's `/maxsim:pause-work` command saves execution state. The WARNING message suggests using it. The CRITICAL message instructs immediate state save.

## Setup

Both hooks are automatically registered during `npx maxsimcli` installation:
    ...
    "PostToolUse": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/maxsim-context-monitor.js"
          }
        ]
      }
    ]
