...
| `git.phase_branch_template` | `"maxsim/phase-{phase}-{slug}"` | Branch template for phase strategy |
| `git.milestone_branch_template` | `"maxsim/{milestone}-{slug}"` | Branch template for milestone strategy |
</config_schema>
...
**Using maxsim-tools.cjs (preferred):**

```bash
# Commit with automatic commit_docs + gitignore checks:
node ~/.claude/maxsim/bin/maxsim-tools.cjs commit "docs: update state" --files .planning/STATE.md
...
**Note:** Most MAXSIM operations use direct file reads or explicit paths, which work regardless of gitignore status.
